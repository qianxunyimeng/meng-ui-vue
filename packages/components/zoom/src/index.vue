  ...
  <div class="mx-zoom">
    ...
    >
      <mx-mask
        ...
        :top="zoomerRect.top"
      />
      <img ref="img" class="origin-img" @load.once="imgLoaded">
      <div
        v-if="selector"
        v-show="!hideZoomer && imgLoadedFlag"
        :class="['img-zoomer', { circle: zoomType === 'circle' && !outZoomer }]"
        :style="[
          zoomerStyle,
          zoomerSize,
          zoomerPosition,
      ...
      >
        <div v-if="outZoomer" class="ks-imagezoom-wrap"></div>
...
<script lang='ts'>
import MxMask from './zoom-mask.vue'
import { getBoundingClientRect } from '@meng-ui-vue/utils/dom'
export default {
  name: 'MxZoom',
  components: {
    MxMask,
  ...
  computed: {
     ...
     */
    zoomerHeight() {
      return this.selectorHeight > 0 ? this.selectorHeight : this.selectorWidth
    },
    /**
     * 缩放器宽/高半径
     */
     ...
     */
    zoomerSize() {
      const { selectorWidth, zoomerHeight: height } = this
      return {
     ...
     */
    outZoomerSize() {
      const { scale, selectorWidth, zoomerHeight: height } = this
      return {
        width: `${selectorWidth * scale}px`,
  ...
  watch: {
     ...
     * 缩放器宽度/高度变化时重置缩放器属性
     */
    selectorWidth: 'initZoomerPoint',
    selectorHeight: 'initZoomerPoint',
